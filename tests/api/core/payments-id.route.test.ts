...
import { GET } from '../../../app/api/core/payments/[id]/route';
...
describe('app/api/core/payments/[id]/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('proxies payment detail via session bridge', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/core/payments/123');
  ...
  it('returns 400 for invalid payment id and does not proxy', async () => {
    const request = new NextRequest('http://localhost:3000/api/core/payments/invalid-id');

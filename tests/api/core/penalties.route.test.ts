...
import { GET } from '../../../app/api/core/penalties/route';
...
describe('app/api/core/penalties/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('proxies penalties list via session bridge', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/core/penalties?status=unpaid&limit=20');
  ...
  it('returns session bridge response on auth failure path', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/core/penalties');
    const response = await GET(request);
  ...
  it('returns 403 for recruiter role before session bridge', async () => {
    const request = new NextRequest('http://localhost:3000/api/core/penalties', {
      headers: {

...
export function useCompanyJobs(options?: UseCompanyJobsOptions) {
  ...
  const fetchJobsPage = useCallback(async (limit: number, offset: number) => {
    try {
      ...
      // Clear stale error envelope after a successful fetch.
      setErrorEnvelope((previous) => (previous ? null : previous));
  ...
  const {
    ...
    refresh,
    hasNextPage,
  } = usePaginatedData<CompanyJob>(fetchJobsPage, pageSize);
